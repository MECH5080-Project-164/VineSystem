...
public:
  ...
  pump_enabled_ = this->get_parameter("pump_enabled").as_bool();
...
private:
  ...
  rcl_interfaces::msg::SetParametersResult parameter_callback(
    ...
    for (const auto & param : parameters) {
      ...
      else if (param.get_name() == "pump_enabled") {
        bool new_state = param.as_bool();
        if (new_state != pump_enabled_) {
          pump_enabled_ = new_state;
          RCLCPP_INFO(this->get_logger(), "pump_enabled parameter set to: %s", pump_enabled_ ? "true" : "false");
          if (!pump_enabled_) {
            // Reset control state when disabling pump
            integral_error_ = 0.0;
            previous_error_ = 0.0;
            // Publish zero PWM to ensure pump is off
            auto pwm_msg = std_msgs::msg::Int32();
            pwm_msg.data = 0;
            pwm_publisher_->publish(pwm_msg);
          }
